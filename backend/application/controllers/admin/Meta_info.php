	...
	function manage(){
		...
		if($this->input->post()){
			...
			if($_FILES['og_image']['name'] != ""){
				$this->removeImage($slug,"og_image",UPLOADIMAGE);
				$image = upload_image(UPLOADIMAGE,"og_image");
			...
			if($_FILES['twitter_image']['name'] != ""){
				$this->removeImage($slug,"twitter_image",UPLOADIMAGE);
				$image = upload_image(UPLOADIMAGE,"twitter_image");
				// pr($image);
